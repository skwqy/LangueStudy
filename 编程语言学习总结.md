...
- **程序计数器**：通常称为PC，在x86-64中用%rip表示，给出简要执行的下一条指令在内存中的地址。
...
| 指令                                   | 参数说明                                 | 描述             |
| -------------------------------------- | ---------------------------------------- | ---------------- |
| iadd、ladd、fadd、dadd                 |                                          | 加法指令         |
| isub、lsub、fsub、dsub                 |                                          | 减法指令         |
| imul、lmul、fmul、dmul                 |                                          | 乘法指令         |
| idiv、ldiv、fdiv、ddiv                 |                                          | 除法指令         |
| irem、lrem、frem、drem                 |                                          | 取余指令         |
| ineg、lneg、fneg、dneg                 |                                          | 取反指令         |
|                                        |                                          |                  |
| ishl、ishr、iushr<br>lshl、lshr、lushr |                                          | 移位指令         |
| iand、land                             |                                          | 按位与指令       |
| ior、lor                               |                                          | 按位或指令       |
| ixor、lxor                             |                                          | 按位异或指令     |
| iinc   INDEX, NUM                      | INDEX：本地变量的编号，NUM：自增减的数量 | 局部变量自增指令 |
|                                        |                                          |                  |
